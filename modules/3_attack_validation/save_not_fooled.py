"""Module 3 — Save patched images that did NOT fool the model to data/not_fooled/.
...
Output:
    data/not_fooled/    <- patched images that did NOT fool the model
...
def save_not_fooled(clean_dir: Path, patched_dir: Path,
    ...
    print(f"\nSaved {saved}/{len(results)} not-fooled patched images to {out_dir}")
...
def main():
    ...
    out_dir     = Path(args.out)     if args.out     else ROOT / "data" / "not_fooled"

    save_not_fooled(clean_dir, patched_dir, out_dir, args.target,
